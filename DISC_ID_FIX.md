...
```bash
python3 -c "from libdiscid.compat import discid; disc = discid.read('/dev/cdrom'); ..."
```
...
python3 -c "from libdiscid.compat import discid; disc = discid.read('/dev/cdrom'); print(disc.id)"
...
```bash
# Fedora/RHEL
sudo dnf install python3-libdiscid

# Debian/Ubuntu  
sudo apt install python3-libdiscid
